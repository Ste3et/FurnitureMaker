...
public class delete {
	...
	public delete(CommandSender sender, Command cmd, String arg2, String[] args){
			...
			if(args.length==2){
				if(args[0].equalsIgnoreCase("delete")){
					...
					if(project!=null){
						if(project.isEditorProject()){
							...
							main.getInstance().deleteMap.put(project, System.currentTimeMillis());
						}else{
				...
				if(args[0].equalsIgnoreCase("delete")){
					...
					if(args[2].equalsIgnoreCase("confirm")){
						...
						if(project!=null){
							if(project.isEditorProject()){
								if(main.getInstance().deleteMap.containsKey(project)){
									Long l1 = main.getInstance().deleteMap.get(project);
									Long l2 = System.currentTimeMillis();
									Long l3 = l2 - l1;
									int sekunden = (int)(l3 / 1000);
									if(sekunden<=20){
										main.getInstance().deleteMap.remove(project);
										List<ObjectID> id = getObject(project);
										if(id!=null){
											for(ObjectID ids : id){
												ids.remove();
